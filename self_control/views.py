...
def index(request):
    ...
    data = {"remembrancies": remembrancies , }

    return render(request, "self_control/index.html", context=data)
...
def index_post(request):
    if request.method == "POST":
        ...
        dat2=" - no no"
        if len(name)==4:
            dat1="Вы ответили правильно"
            if 'both_able_lucubrate_properly' in name:
                ss=1
            if 'output_same' in name:
                ss+=1
            if 'program2' in name:
                ss+=1

            if ss==1:
                dat2=" на один вопрос из 3-х"
            else:
                dat2=f" на {ss} вопроса из 3-х"
            
            sdate=dat1+dat2
                
            data = {"res": sdate,"res1": 5}
        else:
            data = {"res": "Вы выполнили тест неправильно","res1": 0}
            #name3="Вы выполнили тест неправильно"

        return render(request, "self_control/index_post.html", context=data)
